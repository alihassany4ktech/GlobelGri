        ...
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                 ...
                 @if(Auth::guard('admin')->user()->can('create backend role'))
                <div class="col-sm-6">
                    <div class="float-sm-right">
                        <a type="button" href="{{route('admin.backend.role.create')}}"
                                ...
                                <tbody>
                                    @foreach ($roles as $row)
                                        ...
                                        <td class="text-center">
                                            @if(Auth::guard('admin')->user()->can('edit backend role'))
                                            <a href="{{route('admin.backend.role.edit',['id'=>$row->id])}}"
                                                type="button" class="btn btn-sm btn-primary text-white"
                                                data-toggle="tooltip" title="Edit Role">
                                                <i class="fas fa-edit"></i>
                                            </a>
                                            @endif
                                            @if(Auth::guard('admin')->user()->can('delete backend role'))
                                            <a href="{{route('admin.backend.role.delete',['id'=>$row->id])}}"
